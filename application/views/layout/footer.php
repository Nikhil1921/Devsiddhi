<!--   Core JS Files   -->
 ...
 $('#pro_cat_id').on('change',function(){
        ...
        if(categoryID){
            $.ajax({
                type:'POST',
                url: "<?php echo base_url(); ?>" + "admin/project/append_subcategory/"+categoryID,
                success:function(html){ //alert(html);
                    $('#pro_sub_cat_id').html(html);
                }
            }); 
        }else{
            $('#pro_sub_cat_id').html('<option value="">Select Category First... </option>'); 
        }
    });


 $('#descrip-category-select').on('change',function(){
        var descripCategoryID = $(this).val();
        if(descripCategoryID){
            $.ajax({
                type:'POST',
                url: "<?php echo base_url(); ?>" + "admin/project/append_descrip_subcategory/"+descripCategoryID,
                success:function(html){ //alert(html);
                    $('#descrip-sub-category-select').html(html);
                }
            }); 
        }else{
            $('#descrip-sub-category-select').html('<option value="">Select Description Category First... </option>'); 
        }
    });

     document.querySelector('.custom-file-input-1').addEventListener('change',function(e){
      var fileName = document.getElementById("BrochurePDF").files[0].name;
      var nextSibling = e.target.nextElementSibling
      nextSibling.innerText = fileName
    })

       document.querySelector('.custom-file-input-2').addEventListener('change',function(e){
      var fileName = document.getElementById("LayoutPDF").files[0].name;
      var nextSibling = e.target.nextElementSibling
      nextSibling.innerText = fileName
    })

       document.querySelector('.custom-file-input-3').addEventListener('change',function(e){
      var fileName = document.getElementById("CertificatePDF").files[0].name;
      var nextSibling = e.target.nextElementSibling
      nextSibling.innerText = fileName
    })
            /* document.querySelector('.custom-file-input-4').addEventListener('change',function(e){
      var fileName = document.getElementById("MultipleIMAGES").files[0].name;
      var nextSibling = e.target.nextElementSibling
      nextSibling.innerText = fileName
    })*/
